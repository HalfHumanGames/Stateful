...
namespace AIExample {
	...
	public class AI : StateMachine<StateId, ParamId, MessageId> {
		...
		protected override IAddHandlerAddTransitionAddStateBuild<StateId, ParamId, MessageId> GetConfiguration() {
			return Builder.
		...
		private (int x, int y) PatrolGetNextPosition(StateMachine<StateId, ParamId, MessageId> stateMachine, State<StateId, ParamId, MessageId> state, object data) {
			Context context = (Context) data;
			if (context.MonsterPosition.x == path[pathIndex].x && context.MonsterPosition.y == path[pathIndex].y) {
				pathIndex = (pathIndex + 1) % path.Length;
			}
			return MoveTowards(context.MonsterPosition, path[pathIndex]);
		}

		private (int x, int y) AggroGetNextPosition(StateMachine<StateId, ParamId, MessageId> stateMachine, State<StateId, ParamId, MessageId> state, object data) {
			Context context = (Context) data;
			return MoveTowards(context.MonsterPosition, context.PlayerPosition);
		}

		private (int x, int y) LeashGetNextPosition(StateMachine<StateId, ParamId, MessageId> stateMachine, State<StateId, ParamId, MessageId> state, object data) {
			Context context = (Context) data;
			if (context.MonsterPosition.x == path[pathIndex].x && context.MonsterPosition.y == path[pathIndex].y) {
